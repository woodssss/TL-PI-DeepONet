...
class TPIDON():
    ...
    def get_loss(self, tmp_B, tmp_t, tmp_x, tmp_IC):
        ...
        pde = f_t - self.d * f_xx - self.k * f * (tf.ones_like(f) - tf.square(f))
    ...
    def loss_fun_vec(self, W, H, T, T_t, T_xx, Tl, Tr, T_ic, f_ic):
        ...
        pde = np.matmul(K1, W) - self.k * np.matmul(np.matmul(T, H), W) * (np.ones((T.shape[0], 1)) - np.square(np.matmul(np.matmul(T, H), W)))
    ...
    def all_test_result(self, tmp_cl):
    ...
    def all_test_tr_result(self, tmp_cl):
    ...
    def check_with_visual(self):
        ...
        test_evo_error, test_evo_re_error, test_evo_re_error_T, f_pred_tensor, pred_time = self.all_test_result(self.cl)
        _, _, test_evo_re_error_T_sm, _, _ = self.all_test_result(1)
        ...
        test_tr_evo_error, test_tr_evo_re_error, test_tr_evo_re_error_T, f_tr_pred_tensor, pred_tr_time = self.all_test_tr_result(
            self.cl)
        _, _, test_tr_evo_re_error_T_sm, _, _ = self.all_test_tr_result(1)
        ...
        plt.title('TL-DeepONet ' + chr(0x215F + 1))
        xticks = np.arange(0, self.st * self.cl + 1, int(self.st * self.cl / 5))
        labels = np.round(np.arange(0, (self.st * self.cl + 1) * self.dt, int(self.st * self.cl / 5) * self.dt),
                          decimals=1)
        plt.xticks(xticks, labels)
        ...
        plt.title('DeepONet ' + chr(0x215F + 1))
        xticks = np.arange(0, self.st * self.cl + 1, int(self.st * self.cl / 5))
        labels = np.round(np.arange(0, (self.st * self.cl + 1) * self.dt, int(self.st * self.cl / 5) * self.dt),
                          decimals=1)
        plt.xticks(xticks, labels)
...
if __name__ == "__main__":
    ...
    f_test_evo_mat = all_test_evo_mat_load[idx, :st * cl, :]
    ...
    filename = 'C_TPIDON_ac_ct_pb_1d' + '_k_' + num2str_deciaml(k) + '_d_' + num2str_deciaml(
        d) + '_Nx_' + num2str_deciaml(
