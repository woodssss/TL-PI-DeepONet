...
class TPIDON():
    ...
    def get_loss(self, tmp_B, tmp_T, tmp_IC):
        ...
        pde = (f - tmp_IC) - self.dt*self.d * f_xx - self.dt*self.k * f * ( 1 - tf.square(f))
    ...
    def fit(self):
        ...
        for epoch in range(self.num_ad_epochs):
            ...
            if epoch % 5000 == 0:
                ...
                test_loss = self.get_total_loss(self.id_test, self.Nte, 100, Test_B)
    ...
    def loss_fun_vec(self, W, H, T, T_xx, f):
        ...
        pde = np.matmul(K1, W) - self.dt * self.k * np.matmul(np.matmul(T, H), W) * ( 1- np.square(np.matmul(np.matmul(T, H), W))) - f
    ...
    def check_with_visual(self):
        ...
        test_evo_error, test_evo_re_error, test_evo_re_error_T, f_pred_tensor, pred_time = self.all_test_result(self.st)
        _, _, test_evo_re_error_T_sm, _, _ = self.all_test_result(20)
        ...
        test_tr_evo_error, test_tr_evo_re_error, test_tr_evo_re_error_T, f_tr_pred_tensor, pred_tr_time = self.all_test_tr_result(
            self.st)
        _, _, test_tr_evo_re_error_T_sm, _, _ = self.all_test_tr_result(20)

        test_evo_error_avg, test_tr_evo_error_avg = np.sum(test_evo_error, axis=0)/self.Nte, np.sum(test_tr_evo_error, axis=0)/self.Nte
        test_evo_re_error_avg, test_tr_evo_re_error_avg = np.sum(test_evo_re_error, axis=0)/self.Nte, np.sum(test_tr_evo_re_error, axis=0)/self.Nte
        ...
        fig3 = plt.figure(3)
        ax = fig3.add_subplot()
        cp1 = ax.contourf(t_c, x_c, self.f_test_evo_mat.T)
        fig3.colorbar(cp1)
        plt.title('Reference')
        plt.xlabel('t')
        plt.ylabel('x')
        ...
        fig4 = plt.figure(4)
        ax = fig4.add_subplot()
        cp = ax.contourf(t_c, x_c, abs_error_in_time_mat.T, vmax=np.max(abs_error_in_time_mat)*1.2, vmin=0)
        fig4.colorbar(cp1)
        plt.title('DeepONet')
        plt.xlabel('t')
        plt.ylabel('x')
...
if __name__ == "__main__":
    ...
    # st = int(sys.argv[5])
    #
    # d = np.float16(sys.argv[6])
    # k = np.float16(sys.argv[7])
    ...
    filename = 'A_ac_sample_1d_pb_evo' + '_k_' + num2str_deciaml(k) + '_d_' + num2str_deciaml(d) + '_Nx_' + num2str_deciaml(
    ...
    idx=1
    f_test = all_test_f[idx, :][None, :]
    f_test_evo_mat = all_test_evo_mat[idx, :st, :]
    ...
    filename = 'C_TPIDON_ac_dt_1d_pb_evo' + '_k_' + num2str_deciaml(k) + '_d_' + num2str_deciaml(d) + '_Nx_' + num2str_deciaml(
    ...
    mdl = TPIDON(Nx, Ns, Nte, N, k, d, lx, dt, x, dx, Train_B, Test_B, f_test, f_test_evo_mat, all_test_f, all_test_evo_mat, dtype, optimizer, num_ad_epochs,
              num_bfgs_epochs, file_name, fig_name, npy_name, nl, nr, p, q, W_f, bs, ins, train_evo_step, w_bc, st, id_all, id_test)
    mdl.fit()
