################################ Dta generation ch 2d   ##################################
# consider (f^{n+1} - f^n) / \Delta t = \nabla g^{n+1}
# g^{n+1} = -d_1 \nabla f^{n+1} + d_2 f^{n+1}((f^{n+1})^2-1)
# with periodic BC
# Author: Wuzhe Xu
# Date: 07/27/2022
########################################################################################
...
if __name__ == "__main__":
    ...
    # print(f_test.shape, f_test_evo_mat.shape)
    # zxc
    ...
    filename = 'A_sample_ch_2d_pb' + '_d1_' + num2str_deciaml(d1) + '_d2_' + num2str_deciaml(d2) + '_Nx_' + num2str_deciaml(
