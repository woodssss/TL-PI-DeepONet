...
# diffusion equation in discretized time setting using second order in time scheme
...
class TPIDON():
    # initialize
    def __init__(self, Nx, Ns, Nte, N, k, d, lx, dt, x, dx, Train_B, Test_B, f_test, f_test_evo_mat, all_test_f, all_test_f_xx, all_test_evo_mat, dtype, optimizer, num_ad_epochs,
              num_bfgs_epochs, file_name, fig_name, npy_name, nl, nr, p, q, W_f, bs, ins, train_evo_step, w_bc, st, id_all, id_test):
        ...
        self.f_test_xx, self.all_test_f_xx = all_test_f_xx[0, :][None, :], all_test_f_xx
        ...
        #self.id_c = np.array(random.sample(range(Nx), 50))
        ## every other grid
    ...
    def fit(self):
        ...
        #self.compare_loss(self.all_test_f[[2], :])
    ...
    def loss_fun_vec(self, W, H, T, T_xx, f, f_xx):
        W = W[:, None]
        K1 = np.matmul(T, H) - self.dt * self.d /2 * np.matmul(T_xx, H)

        RHS = f + self.k*self.dt/2*np.square(f) + self.dt*self.d/2*f_xx

        pde = np.matmul(K1, W) - self.dt * self.k/2 * np.square(np.matmul(np.matmul(T, H), W)) - RHS

        F = np.concatenate([pde], axis=0)[:, 0]

        return F.tolist()

    # def compare_loss(self, f_old):
    #     # loss 1
    #     with tf.GradientTape(persistent=True) as tape:
    #         tape.watch(self.x)
    #         B = self.Bnn(f_old)[:, :, 0]
    ...
    #
    #     pde_1 = (f - f_old.T) - self.dt*self.d * f_xx - self.dt*self.k * tf.square(f)
    #
    #     # loss 2
    #
    #     weights_B = self.Bnn.get_weights()
    ...
    #
    #     H = self.Hnn(f_old)[0, :, :].numpy()
    #
    #     pde_2 = self.loss_fun_vec(self.W_f, H, T, T_xx, f_old.T)
    #
    #     print(np.max(np.abs(pde_1.numpy() - np.array(pde_2)[:, None])))
    ...
    def get_Tevo_pred_c(self, T, T_xx, T_c, T_xx_c, f, f_xx):
        ...
        for i in range(self.st):
            ...
            f_xx_c = f_xx[self.id_c, :]

            res = scipy.optimize.leastsq(self.loss_fun_vec, W_tmp, args=(H, T_c, T_xx_c, f_c, f_xx_c), ftol=1e-5,
                                         xtol=1e-5)

            W_tmp = res[0][:, None]

            f_next = np.matmul(np.matmul(T, H), W_tmp)
            f_xx_next = np.matmul(np.matmul(T_xx, H), W_tmp)
    ...
    def all_test_result(self):
        ...
        re_error_T = np.sqrt(np.sum(np.square(f_pred_tensor - self.all_test_evo_mat))/np.sum(np.square(self.all_test_evo_mat)))
    ...
    def all_test_tr_result(self):
        ...
        for i in range(self.Nte):
            tmp_f = self.all_test_f[[i], :]
            tmp_f_xx = self.all_test_f_xx[[i], :]

            _, f_tmp_pred = self.get_Tevo_pred_c(T, T_xx, T_c, T_xx_c, tmp_f, tmp_f_xx)
    ...
    def check_with_visual(self):
        ...
        #f_tr_0, f_tr_evo_mat = self.get_Tevo_pred(T, T_xx, self.f_test)
        f_tr_0, f_tr_evo_mat = self.get_Tevo_pred_c(T, T_xx, T_c, T_xx_c, self.f_test, self.f_test_xx)
        ...
        test_evo_error_avg, test_tr_evo_error_avg = np.sum(test_evo_error, axis=0)/self.Nte, np.sum(test_tr_evo_error, axis=0)/self.Nte
        test_evo_re_error_avg, test_tr_evo_re_error_avg = np.sum(test_evo_re_error, axis=0)/(self.Nte-2), np.sum(test_tr_evo_re_error, axis=0)/(self.Nte-2)
...
if __name__ == "__main__":
    ...
    filename = 'A_nrd_sample_1d_bigt_evo' + '_k_' + num2str_deciaml(k) + '_d_' + num2str_deciaml(d) + '_Nx_' + num2str_deciaml(
    ...
    for i in range(Nte):
        plt.plot(x, all_test_f[i, :], 'r')
        plt.plot(x, all_test_f_xx[i, :], 'b')
    ...
    filename = 'C_nrd_dt_1d_2nd_evo' + '_k_' + num2str_deciaml(k) + '_d_' + num2str_deciaml(d) + '_Nx_' + num2str_deciaml(
    ...
    mdl = TPIDON(Nx, Ns, Nte, N, k, d, lx, dt, x, dx, Train_B, Test_B, f_test, f_test_evo_mat, all_test_f, all_test_f_xx, all_test_evo_mat, dtype, optimizer, num_ad_epochs,
              num_bfgs_epochs, file_name, fig_name, npy_name, nl, nr, p, q, W_f, bs, ins, train_evo_step, w_bc, st, id_all, id_test)
    mdl.fit()
