...
class TLPIDON():
    ...
    def get_B(self):
        ...
        UB = tf.keras.layers.Dense(units=self.nr, activation=tf.nn.tanh,
                                   kernel_initializer='glorot_normal', kernel_regularizer=tf.keras.regularizers.L2(0.001))(
            input_Branch)
    ...
    def get_Tevo_pred_c(self, T, T_c, T_xx_c, f, tmp_st):
        ...
        for i in range(tmp_st):
            ...
            res = scipy.optimize.leastsq(self.loss_fun_vec, W_tmp, args=(H, T_c, T_xx_c, f_c), ftol=1e-6/2,
                                         xtol=1e-6/2)
    ...
    def all_test_result(self, tmp_st):
        ...
        re_error_T = np.sqrt(np.sum(np.square(f_pred_tensor - self.all_test_evo_mat[:, :tmp_st, :]))/np.sum(np.square(self.all_test_evo_mat[:, :tmp_st, :])))
    ...
    def check_with_visual(self):
        ...
        test_tr_evo_error, test_tr_evo_re_error, test_tr_evo_re_error_T, f_tr_pred_tensor, pred_tr_time = self.all_test_tr_result(self.st)
        ...
        with open(self.file_name, 'a') as fw:
            print('Avg long time PIDON test l2 error to 1: %.3e, Avg long time PIDON test l2 relative error to end: %.3e' %
...
if __name__ == "__main__":
    ...
    filename = 'D_TPIDON_nrd_dt_1d_evo' + '_k_' + num2str_deciaml(k) + '_d_' + num2str_deciaml(d) + '_Nx_' + num2str_deciaml(
